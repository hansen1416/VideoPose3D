...
dataset = CustomDataset("custom_dataset/" + args.dataset + ".npz")
...
keypoints = np.load("custom_dataset/" + args.dataset + ".npz", allow_pickle=True)
...
if not args.evaluate:
    ...
    if semi_supervised:
        cameras_semi, _, poses_semi_2d = fetch(
            subjects_semi, action_filter, parse_3d_poses=False
        )
        ...
        optimizer = optim.Adam(
            list(model_pos_train.parameters()) + list(model_traj_train.parameters()),
            lr=lr,
            amsgrad=True,
        )
    ...
    if semi_supervised:
        semi_generator = ChunkedGenerator(
            args.batch_size // args.stride,
            cameras_semi,
            None,
            poses_semi_2d,
        ...
        semi_generator_eval = UnchunkedGenerator(
            cameras_semi,
            None,
            poses_semi_2d,
            pad=pad,
        ...
        print(
            "INFO: Semi-supervision on {} frames".format(
                semi_generator_eval.num_frames()
            )
        )
    ...
    while epoch < args.epochs:
        ...
        if semi_supervised:
            ...
            for (_, batch_3d, batch_2d), (cam_semi, _, batch_2d_semi) in zip(
                train_generator.next_epoch(), semi_generator.next_epoch()
            ):
                ...
                inputs_2d_cat = (
                    torch.cat((inputs_2d, inputs_2d_semi), dim=0)
                    if not skip
                    else inputs_2d
                )
                ...
                loss_traj = weighted_mpjpe(
                    predicted_traj_cat[:split_idx], inputs_traj, w
                )
                epoch_loss_traj_train += (
                    inputs_3d.shape[0] * inputs_3d.shape[1] * loss_traj.item()
                )
                assert (
                    inputs_traj.shape[0] * inputs_traj.shape[1]
                ...
                if not skip:
                    ...
                    projection_func = (
                        ...
                        if args.linear_projection
                        else project_to_2d
                    )
                    reconstruction_semi = projection_func(
                        predicted_semi + predicted_traj_cat[split_idx:], cam_semi
                    )

                    loss_reconstruction = mpjpe(
                        reconstruction_semi, target_semi
                    )  # On 2D poses
                    epoch_loss_2d_train_unlabeled += (
                        predicted_semi.shape[0]
                        * predicted_semi.shape[1]
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d in train_generator_eval.next_epoch():
                    ...
                    if semi_supervised:
                        ...
                        epoch_loss_2d_train_labeled_eval += (
                            reconstruction.shape[0]
                            * reconstruction.shape[1]
                            * loss_reconstruction.item()
                        )
                ...
                if semi_supervised:
                    ...
                    losses_2d_train_labeled_eval.append(
                        epoch_loss_2d_train_labeled_eval / N
                    )
                ...
                if semi_supervised:
                    for cam, _, batch_2d in semi_generator_eval.next_epoch():
                        ...
                        if pad > 0:
                            ...
                            target_semi = inputs_2d_semi[:, :, :, :2].contiguous()
                        reconstruction_semi = project_to_2d(
                            predicted_3d_pos_semi + predicted_traj_semi, cam
                        )
                        loss_reconstruction_semi = mpjpe(
                            reconstruction_semi, target_semi
                        )

                        epoch_loss_2d_train_unlabeled_eval += (
                            reconstruction_semi.shape[0]
                            * reconstruction_semi.shape[1]
                            * loss_reconstruction_semi.item()
                        )
                        N_semi += (
                            reconstruction_semi.shape[0] * reconstruction_semi.shape[1]
                        )
                    losses_2d_train_unlabeled_eval.append(
                        epoch_loss_2d_train_unlabeled_eval / N_semi
                    )
        ...
        if args.no_eval:
            ...
            if semi_supervised:
                print(
                    "[%d] time %.2f lr %f 3d_train %f 3d_eval %f traj_eval %f 3d_valid %f "
        ...
        momentum = initial_momentum * np.exp(
            -epoch / args.epochs * np.log(initial_momentum / final_momentum)
        )
            ...
            torch.save(
                {
                    ...
                    "model_traj": (
                        model_traj_train.state_dict() if semi_supervised else None
                    ),
                    "random_state_semi": (
                        semi_generator.random_state() if semi_supervised else None
                    ),
                },
                chk_path,
            )
        ...
        if args.export_training_curves and epoch > 3:
            ...
            if semi_supervised:
                ...
                plt.legend(
                    [
                        "2d train labeled (eval)",
                        "2d train unlabeled",
                        "2d train unlabeled (eval)",
                        "2d valid (eval)",
                    ]
                )
                plt.ylabel("MPJPE (2D)")
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            epoch_loss_3d_pos_procrustes += (
                inputs_3d.shape[0]
                * inputs_3d.shape[1]
                * p_mpjpe(predicted_3d_pos, inputs)
            )
